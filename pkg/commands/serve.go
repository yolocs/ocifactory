...
func registerServeFlags(flags *pflag.FlagSet) {
	...
	flags.Int64(flagNPMMaxUploadBytes, npm.DefaultMaxUploadBytes,
		"Cap on the total request-body size accepted by the npm publish "+
			"endpoint. npm publish bodies are JSON with the tarball "+
			"base64-encoded inline, so the wire size is ~1.34x the tarball "+
			"size. Defends against an authenticated client streaming arbitrary "+
			"bytes to burn instance hours / egress before the OCI backend "+
			"rejects the layer. Set to 0 to disable the cap.")
